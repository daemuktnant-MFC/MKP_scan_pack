...
if not st.session_state.current_user_name:
    ...
    # ================= MODE 3: MANAGE USERS =================
        ...
        # --- SECTION: ADD USER (FIXED) ---
        with col_add:
            st.subheader("➕ เพิ่มพนักงานใหม่")
            
            # 1. สร้าง Input Fields (ไม่ใช้ st.form แล้ว เพื่อให้คุม State ได้ง่าย)
            new_id = st.text_input("รหัสพนักงาน (ID)", placeholder="เช่น 001", key="input_new_id")
            new_name = st.text_input("ชื่อ-นามสกุล", placeholder="เช่น สมชาย ใจดี", key="input_new_name")
            new_pass = st.text_input("รหัสผ่าน (Password)", type="password", key="input_new_pass")
            role_option = st.selectbox("สิทธิ์การใช้งาน (Role)", ["staff", "admin"], key="input_new_role")
            
            # 2. ฟังก์ชันสำหรับปุ่มกด (Callback)
            def click_add_user():
                # ดึงค่าจาก State โดยตรง
                uid = st.session_state.input_new_id
                uname = st.session_state.input_new_name
                upass = st.session_state.input_new_pass
                urole = st.session_state.input_new_role
                
                if uid and uname and upass:
                    success, msg = add_new_user_to_sheet(uid, upass, uname, urole)
                    if success:
                        st.toast(msg, icon="✅")
                        # ล้างค่าในช่องเมื่อบันทึกสำเร็จ
            ...
            # 3. ปุ่มกดเรียกฟังก์ชัน
            st.button("บันทึกข้อมูล", type="primary", use_container_width=True, on_click=click_add_user)
        ...
        with col_del:
            ...
            if not df_users_manage.empty:
                if len(df_users_manage.columns) >= 3:
                     # เช็ค Error column ก่อนสร้าง dropdown
                     try:
                         ...
                         # ใช้ Callback สำหรับปุ่มลบเช่นกัน
                         def click_del_user():
                         ...
                         st.button("ยืนยันการลบ", type="secondary", use_container_width=True, on_click=click_del_user)
                     except Exception as e:
                         st.error(f"เกิดข้อผิดพลาดในการแสดงรายชื่อ: {e}")
                else:
